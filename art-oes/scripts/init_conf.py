...
def write_zb_server():
    if CONF['is_enable_oes_zb'] == 'yes':
        oes_zb_info = {
            'ansible_ssh_host': CONF['oes_zb_host'],
            'ansible_ssh_port': CONF['oes_zb_port'],
            'ansible_ssh_user': CONF['oes_zb_user'],
            'zb_deploy_path': CONF['oes_zb_path'],
        }
        with open(os.path.join(var_file_path, 'zb_oes.yml'), 'w', encoding='utf-8') as oes_file:
            yaml.dump(oes_zb_info, oes_file, default_flow_style=False, encoding='utf-8', allow_unicode=True)
