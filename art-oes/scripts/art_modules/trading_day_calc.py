...
def main():
    ...
    date = options.date
    pre_trd = options.pre.strip()
    next_trd = options.next.strip()

    if date:
        print(get_curr_date())
    elif pre_trd:
        the_year = int(pre_trd[:4])
        trdDateList = trd_date_info.get('trd_date_%s_list' % the_year, '')
        print(pre_trd_date(pre_trd, trdDateList, the_year))
    elif next_trd:
        the_year = int(next_trd[:4])
        trdDateList = trd_date_info.get('trd_date_%s_list' % the_year, '')
        print(next_trd_date(next_trd, trdDateList, the_year))
