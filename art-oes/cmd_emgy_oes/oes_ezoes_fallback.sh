...
_ezoesfallback()
{
	#执行ezoesfallback.sh回退至ezoes报盘！
	echo "开始执行ezoesfallback.sh回退流网关至ezoes报盘模式"
	ssh -t -t -p $sh_port $oes_user@$oes_dep_Lip "cd host_01/${oes_dep_name}/bin && ./sse_tdgw_mtp_declarer stop && ./ezoes_declarer stop &&./ezoes_declarer fallback && ./ezoes_declarer start "
	ssh -t -t -p $sh_port $oes_user@$oes_dep_Fip "cd host_02/${oes_dep_name}/bin && ./sse_tdgw_mtp_declarer stop && ./ezoes_declarer stop &&./ezoes_declarer fallback && ./ezoes_declarer start "
	echo "上海报盘模式已回退至ezoes，请检查相关进程状态是否正常"
}
