...
##############################################################
# Name: _cleartxstart
# Desc: clear txlog, start oes, load oes
# Args:
##############################################################
_cleartxstart()
{
	#停止OES,并清除TXLOG数据
	echo "主备仲裁模式停止oes,并清除TXLOG数据"
...
_isexecute()
{
	case "$1" in
	cleartx|ct)
	echo "您好，准备开始执行重启交易机器脚本，清除txlog重启，请耐心等待！"
	_cleartxstart
	;;
	nocleartx|nct)
	echo "您好，准备开始执行重启交易机器脚本，带txlog重启，请耐心等待！"
	_restart
	;;
	...
	*)
	echo ""
	echo " 非有效指令，请重新执行命令，可以输入./oes_restart.sh -h 查看帮助！"
	echo ""
	exit;;
	esac
}
...
helpdesc()
{
	...
	echo ""
	echo "Commands are:"
	echo "    参数1：是否清除交易日志重启，取值：cleartx/ct:备份及清除日志重启; nocleartx/nct:不清除日志重启;"
	echo "=============================================================================="
	echo ""
	echo "执行命令例如："
	echo "        ./oes_restart.sh cleartx"
	echo "等同于：./oes_restart.sh ct"
	echo ""
}
