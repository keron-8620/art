...
_killallmds()
{
	#执行KillMDS.sh强杀oes所有进程！
	echo "开始执行KillMDS.sh强杀oes所有进程"
	ssh -t -t -p $sh_port $mds_user@$mds_dep_Lip "cd host_01/${mds_dep_name}/sbin;sh KillMDS.sh -Y"
	ssh -t -t -p $sh_port $mds_user@$mds_dep_Fip "cd host_02/${mds_dep_name}/sbin;sh KillMDS.sh -Y"
	ssh -t -t -p $sh_port $mds_user@$mds_dep_Aip "cd host_03/${mds_dep_name}/sbin;sh KillMDS.sh -Y"
	echo "执行KillMDS.sh强杀oes所有进程结束"
}
