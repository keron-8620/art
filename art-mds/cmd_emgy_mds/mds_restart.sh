...
##############################################################
# Name: _fixstart
# Desc: clear txlog, start mds, load mds
# Args:
##############################################################
_cleartxstart()
{
	#停止MDS,并清除TXLOG数据
	echo "主备仲裁模式停止mds,并清除TXLOG数据"
	ssh -t -t -p $sh_port $mds_user@$mds_dep_Aip "cd host_03/${mds_dep_name}/bin;./mds stop -F"
	ssh -t -t -p $sh_port $mds_user@$mds_dep_Lip "cd host_01/${mds_dep_name}/bin;./mds stop -F"
	ssh -t -t -p $sh_port $mds_user@$mds_dep_Fip "cd host_02/${mds_dep_name}/bin;./mds stop -F"

	#启动MDS
	ssh -t -t -p $sh_port $mds_user@$mds_dep_Lip "cd host_01/${mds_dep_name}/bin;./mds start;./mds taskstatus -k reset -S 5"
	sleep 15
	ssh -t -t -p $sh_port $mds_user@$mds_dep_Fip "cd host_02/${mds_dep_name}/bin;./mds start;./mds taskstatus -k reset -S 5"
	ssh -t -t -p $sh_port $mds_user@$mds_dep_Aip "cd host_03/${mds_dep_name}/bin;./mds start;./mds taskstatus -k reset -S 5"

	echo "重启启动命令执行完成，请及时去MON检查mds的状态"
}
