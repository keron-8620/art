...
_gentestdata()
{
	...
	#2、复制数据，制造的data数据
	#   拷贝到mds的生产运行目录上面(/home/mdsuser/host_01/mds-0.15.9-release)
	#   从mds主机拷贝数据回art到临时目录下面，并删除mds里面的刚才解压处理的临时数据
	#   从art主机将mds的data数据分发到mds备机器和mds仲裁机上面
	#   删除art主机上面的临时data数据
	...
	#4、制造ok文件，执行cmd_emgency_mds里面的mds_okflag.sh脚本

	tradedate_full=$1
	tradedate=$(echo $tradedate_full | cut -c 5-8)

	# 获得测试数据备份文件名称
	testdata=$(ssh -t -t -p $sh_port $mds_user@$mds_dep_Lip "find /home/mdsuser/host_01/backup/ -name data-*.tar.gz -type f -print | sort -b -k1 -r | awk 'NR==1{print}';" |sed 's/\r//' )
	...
	ssh -t -t -p $sh_port $mds_user@$mds_dep_Lip "cd host_01/backup;tar -zxvf ${testdata};cd data;rename ${oldymd} ${tradedate_full} flags/*;rename ${oldmd}.txt ${tradedate}.txt exchange/*;rename ${oldymd}.xml ${tradedate_full}.xml exchange/*;sed -i -e \"s|EffectiveDate.*|EffectiveDate = $nowdate_full|\" TradingDate.txt;sed -i -e \"s|TradingDate.*|TradingDate = $tradedate_full|\" TradingDate.txt;cd ..;tar -zcvf testdata_${tradedate_full}.tar.gz data;cp -rf data ../../host_01/${mds_dep_name}/.;rm -rf data;"
	echo "主机数据生成脚本已经执行完毕，请检查！"

	echo "开始分发数据到备机和仲裁机"
	cd $basepath/files
	scp ${mds_user}@${mds_dep_Lip}:/home/${mds_user}/host_01/backup/testdata_${tradedate_full}.tar.gz .
	tar -zxvf testdata_${tradedate_full}.tar.gz
	scp -r data ${mds_user}@${mds_dep_Fip}:/home/${mds_user}/host_02/${mds_dep_name}/.
	scp -r data ${mds_user}@${mds_dep_Aip}:/home/${mds_user}/host_03/${mds_dep_name}/.
	...
	ssh -t -t -p $sh_port $mds_user@$mds_dep_Lip "cd host_01/${mds_dep_name}/data;sed -i -e \"s|EffectiveDate.*|EffectiveDate = $nowdate_full|\" TradingDate.txt;sed -i -e \"s|TradingDate.*|TradingDate = $tradedate_full|\" TradingDate.txt;"
	ssh -t -t -p $sh_port $mds_user@$mds_dep_Fip "cd host_02/${mds_dep_name}/data;sed -i -e \"s|EffectiveDate.*|EffectiveDate = $nowdate_full|\" TradingDate.txt;sed -i -e \"s|TradingDate.*|TradingDate = $tradedate_full|\" TradingDate.txt;"
	ssh -t -t -p $sh_port $mds_user@$mds_dep_Aip "cd host_03/${mds_dep_name}/data;sed -i -e \"s|EffectiveDate.*|EffectiveDate = $nowdate_full|\" TradingDate.txt;sed -i -e \"s|TradingDate.*|TradingDate = $tradedate_full|\" TradingDate.txt;"

	echo "生成OK文件"
	cd $basepath/cmd_emgy_mds
	sh mds_okflag.sh ${tradedate_full}
	...
	*)
	if [ -z "$1" ];then
		...
		if [ $? = 0 ]; then
			...
			echo ""
			echo " 非有效指令，请重新执行命令，可以输入./mds_testdata_make.sh -h 查看帮助！"
			echo ""
		fi 
	fi
	exit;;
...
helpdesc()
{
	...
	echo ""
	echo "执行命令例如："
	echo "        ./mds_testdata_make.sh 20191111"
	echo ""
}
