...
_genokflag()
{
	...
	#手工生成ok标志文件
	echo "开始手工生成ok标志文件"
	lresok=$(ssh -t -t -p $sh_port $mds_user@$mds_dep_Lip "cd host_01/${mds_dep_name}/data/flags;> DATA_FETCH_MON.${nowdate_full}.OK;> DATA_FETCH_SSE.${nowdate_full}.OK;> DATA_FETCH_SZSE.${nowdate_full}.OK;ls |grep -i ${nowdate_full}")
	echo "主机标志文件已经生成："
	echo "${lresok}"

	fresok=$(ssh -t -t -p $sh_port $mds_user@$mds_dep_Fip "cd host_02/${mds_dep_name}/data/flags;> DATA_FETCH_MON.${nowdate_full}.OK;> DATA_FETCH_SSE.${nowdate_full}.OK;> DATA_FETCH_SZSE.${nowdate_full}.OK;ls |grep -i ${nowdate_full}")
	echo "备机标志文件已经生成："
	echo "${fresok}"

	aresok=$(ssh -t -t -p $sh_port $mds_user@$mds_dep_Aip "cd host_03/${mds_dep_name}/data/flags;> DATA_FETCH_MON.${nowdate_full}.OK;> DATA_FETCH_SSE.${nowdate_full}.OK;> DATA_FETCH_SZSE.${nowdate_full}.OK;ls |grep -i ${nowdate_full}")
	echo "仲裁机标志文件已经生成："
	echo "${aresok}"
	...
	*)
	if [ -z "$1" ];then
		...
		if [ $? = 0 ]; then
			...
			echo ""
			echo " 非有效指令，请重新执行命令，可以输入./mds_okflag.sh -h 查看帮助！"
			echo ""
		fi 
	fi
	exit;;
